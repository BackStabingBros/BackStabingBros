--[[ 
    Script coded by blitzisking#0 and gladiator_0315#0
...
print("Preparing to load script from: " .. c)
...
l_fastload_enabled = function(e)
    if e == "flush" then
        ...
        local h, i = pcall(function()
            g = game:HttpGet(c)
            print("Got script content, saving to file...")
        ...
        if not h or not f then
            print("Error while loading script: " .. tostring(i))
...
if not k or not j then
    print("Cache file is empty or invalid, flushing...")
    l_fastload_enabled("flush")
    return
end

print("Executing cached script...")
j(is_from_loader)
